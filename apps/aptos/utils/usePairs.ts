      ...
      () =>
        pairAddresses.map((pairAddress) => ({
          ...
          queryFn: async () => {
            ...
            const response = await fetch(`${FETCH_URL_PREFIX}/v1/accounts/${CONTRACT_ADDRESS}/resource/${pairAddress}`)
          ...
          queryKey: [{ entity: 'pairResource', pairAddress, resourceType: CONTRACT_ADDRESS }],
          staleTime: Infinity,
...
export default function usePairs(currencies: [Token | undefined, Token | undefined][]): [PairState, Pair | null][] {
...
function getPair(token0: Token, token1: Token, currencyAmountA: string, tokenAmountB: string) {
